    ...
    def compute(self, img1_path: Path, img2_path: Path, 
        ...
        try:
            ...
            with self.torch.no_grad():
                flow_predictions = self.model(img1, img2, iters=32)
...
def run_comparison_pipeline(args):
    ...
    for seq in sequences:
        ...
        print(f"  {seq}: {len(pairs)} pairs")
    
    print(f"Total: {len(all_pairs)} frame pairs")
    
    # Limit number of pairs for testing
    if args.max_pairs:
        all_pairs = all_pairs[:args.max_pairs]
        print(f"Limited to {len(all_pairs)} pairs for testing")
